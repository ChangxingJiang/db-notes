...
|---------|---------|---------|-----------|------|
| `PPDSolver` | 谓词优化 | 谓词下推 | Predicate Push Down | 将过滤条件尽可能下推到数据源，减少数据扫描量 |
| `PredicateSimplification` | 谓词优化 | 谓词简化 | Predicate Simplification | 简化复杂过滤条件，合并冗余表达式 |
| `ConstantPropagationSolver` | 谓词优化 | 常量传播 | Constant Propagation Solver | 传播常量值，简化表达式 |
...
| `ColumnPruner` | 列优化 | 列裁剪 | Column Pruning | 移除计划中不需要的列 |
| `GcSubstituter` | 列优化 | 生成列替换 | Constant Propagation | 使用常量值替换表达式 |
